<template>
	<view class="uni-searchbar">
		<view :style="{borderRadius:radius+'px',backgroundColor: bgColor}" class="uni-searchbar__box" @click="searchClick">
			...
			<!-- #endif -->
			<input v-if="true" :focus="showSync" :placeholder="placeholder" :maxlength="maxlength" @confirm="confirm" class="uni-searchbar__box-search-input"
			 confirm-type="search" type="text" v-model="searchVal" />
			<text v-else class="uni-searchbar__text-placeholder">{{ placeholder }}</text>
			<view v-if="false" class="uni-searchbar__box-icon-clear" @click="clear">
...
<style lang="scss" scoped>
	...
	.uni-searchbar {
		...
		background-color: #333333; // 已修改背景色
	}
